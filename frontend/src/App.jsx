...
const App = () => {
  ...
  const getQuestion = async (questionId) => {
    if (contract && questionId) {
      try {
        const [asker, content, timestamp, tag, bestAnswerId] =
  ...
  const renderRegisterPage = () => (
    <div className="flex flex-col justify-center items-center">
      ...
      />
      <Button className="text-xl h-14 p-3 mt-7 " onClick={registerUser}>
        Register
      </Button>
    </div>
  );
  ...
  const renderForumPage = () => (
    <div className="text-2xl flex flex-col gap-9">
      <Card className="min-w-[20vw] flex flex-col justify-center items-center mt-10">
        <CardHeader>
        ...
        />
        <CardFooter>
          <Button className="mt-7" onClick={addTag}>
            Add Tag
          </Button>
        </CardFooter>
      </Card>
      ...
      <ul>
        {questions.map((q, index) => (
          <li key={index} onClick={() => getAnswersByQuestionId(q[0])}>
            {q}
          </li>
        ))}
      </ul>
